...
class DDNS:
    def __init__(self, api_id, api_token):
        ...
        self.data = {"login_token": self.login_token,
                     "format": self.format, "lang": self.lang}
    ...
    def get_record_list(self):
        ...
        for i in self.domain_list:
            ...
            r = json.loads(requests.post(
                url=url, data=data, headers=self.headers).text)
            for j in r["records"]:
    ...
    def update(self):
        ...
        for i in record:
            if(i["value"] == new_ip):
                ...
                continue
            d = self.data.copy()
            d.update({"domain": i["domain"], "record_id": i["id"], "sub_domain": i["name"],
                      "record_type": "A", "record_line": i["line"], "value": new_ip})
            r = json.loads(requests.post(
                url=url, data=d, headers=self.headers).text)
            if (r["status"]["code"] == "1"):
